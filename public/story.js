...
export function recentChanges(before, after) {
  ...
  return {
    ...
    parties: Object.keys(after.shares).filter(key => Number.isFinite(before.shares[key]) && Number.isFinite(after.shares[key])).map(key => ({ key, delta: after.shares[key] - before.shares[key] })).sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta))
  };
}
export function milestones(points, { regional = false } = {}) {
  const events = []; let previous, leader = 0, highWater = 0;
